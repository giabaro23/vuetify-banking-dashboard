<template>
  <v-card color="#202022" class="rounded-lg pb-1 mt-7" flat>
    ...
    >
      <v-toolbar flat color="transparent">
        <v-toolbar-title class="grey--text caption"
          >Universal Card</v-toolbar-title
        >
        <v-spacer></v-spacer>
    ...
    <v-list class="mt-n6" color="transparent" dense>
      <v-list-item v-for="chat in chats" :key="chat.title">
        ...
        <v-list-item-content>
          <v-list-item-title v-text="chat.title" class="white--text">
          </v-list-item-title>
          <v-list-subtitle
    ...
    <v-card color="#242426" class="rounded-lg mx-2 pa-1 mt-n3 gradient flip">
      ...
      <v-card-text class="pt-0 mt-5">
        <v-btn color="#524EED" dark class="withouupercase mt-2">Trial from $1999</v-btn>
      </v-card-text>
    </v-card>
  </v-card>
</template>
